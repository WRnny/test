...
int main(void)
{
	...
	{			
		...
		if(Flag_mode == 2 && Flag_A2 == 1)
		{
			...
			float temp_error = Basic_set_yaw - Yaw_received;
					while(temp_error > 180.0f) temp_error -= 360.0f;
					while(temp_error < -180.0f) temp_error += 360.0f;
					if(ABS(temp_error) < 1 && time_A6 == 1)
			{
				encoder_r_sum = 0;
				encoder_l_sum = 0;
				time_A6++;
		...
		//A2过直角判断
		float temp_error = Yaw_received - Basic_set_yaw;
				while(temp_error > 180.0f) temp_error -= 360.0f;
				while(temp_error < -180.0f) temp_error += 360.0f;
				if(Flag_A2 == 3 && Flag_mode == 2 && temp_error > 80)
		{
			Flag_mode = 0;
		...
		if(Flag_mode == 4 && Flag_A4 == 6)
		{
			...
			float temp_error = Basic_set_yaw - Yaw_received;
					while(temp_error > 180.0f) temp_error -= 360.0f;
					while(temp_error < -180.0f) temp_error += 360.0f;
					if(ABS(temp_error) < 10 && time_A6 == 1)
			{
				encoder_r_sum = 0;
				encoder_l_sum = 0;
				time_A6++;
