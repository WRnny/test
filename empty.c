...
int Basic_set_speed = 530;//电机基础速度	500
...
int main(void)
{
	...
	{			
		...
		if(Flag_mode == 3 && Flag_A3 == 2 && Flag_coord > 100 && encoder_r_sum >1500 && encoder_l_sum > 1500)
		{
		...
		// 经过干扰路段，开启循迹模式
		if(Flag_mode == 4 && Flag_A4 == 2 && encoder_r_sum >1000 && encoder_l_sum > 1000)
		{
		...
		if(Flag_mode == 4 && Flag_A4 == 3 && encoder_r_sum >100 && encoder_l_sum > 100 && Flag_RigjtAngle == 1)
		{
			encoder_r_sum = 0;
			encoder_l_sum = 0;
		...
		if(Flag_mode == 4 && Flag_A4 == 6)
		{
			...
			if(ABS(Basic_set_yaw - Yaw_received) < 5 && time_A6 == 1)
			{
				encoder_r_sum = 0;
				encoder_l_sum = 0;
				time_A6++;
		...
		if(Flag_mode == 4 && Flag_A4 == 7 && encoder_r_sum >1000 && encoder_l_sum > 1000 && Flag_coord > 10000)
		{
			encoder_r_sum = 0;
			encoder_r_sum = 0;
		...
		if(Flag_mode == 4 && Flag_A4 == 9 && Flag_Black == 1)
		{
			Flag_mode = 0;
...
//		}
		...
		if(Flag_yawmode == 2 && Flag_A5 == 1)
		{
			...
			if(Basic_set_yaw - Yaw_received < 3)
			{
				encoder_r_sum = 0;
				encoder_l_sum = 0;
		...
		if(laps && Flag_A6set == 2 && Flag_A6 == 2)
		{
			...
			if(Basic_set_yaw - Yaw_received < 1)
			{
				encoder_r_sum = 0;
				encoder_l_sum = 0;
...
//				if (Track < -300.0f) Track = -300.0f;
			...
			if(Flag_A2 == 1 || Flag_A3 == 1 || Flag_A3 == 3 || Flag_A4 == 1 || Flag_A4 == 4 || Flag_A4 == 6)
			{
