...
//DL_GPIO_writePinsVal(GPIO_LED_PORT,OLED_OLED_SDA_PIN,x);

void OLED_W_SCL(uint8_t x)
{
	if(x)
	{
		DL_GPIO_setPins(OLED_OLED_SCL_PORT,OLED_OLED_SCL_PIN);
	}
	else
	{
		DL_GPIO_clearPins(OLED_OLED_SCL_PORT,OLED_OLED_SCL_PIN);
	}
	delay_cycles(3);
}
void OLED_W_SDA(uint8_t x)
{
	if(x)
	{
		DL_GPIO_setPins(OLED_OLED_SDA_PORT,OLED_OLED_SDA_PIN);
	}
	else
	{
		DL_GPIO_clearPins(OLED_OLED_SDA_PORT,OLED_OLED_SDA_PIN);
	}
	delay_cycles(3);
}
